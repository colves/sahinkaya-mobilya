...
function arayuzuGuncelle() {
    try {
        ...
        setDisp('filtre-alani', ayarlar.filtreAktif ? 'block' : 'none');
        ...
        const anasayfada = location.pathname.endsWith('/') || location.pathname.endsWith('index');
        if (anasayfada) {
...
function urunAra() {
    ...
    if(q) location.href = url('kategori') + '?ara=' + encodeURIComponent(q);
}
...
function aramaOnerileriGetir() {
    ...
    if (sonuclar.length === 0) {
        ...
        const detayBase = url('urun-detay');
        kutu.innerHTML = sonuclar.slice(0, 6).map(u => {
            ...
            <div style="display:flex; align-items:center; gap:10px; padding:10px 15px; border-bottom:1px solid var(--kenarlik); cursor:pointer; transition:var(--gecis);" 
                 ...
                 onclick="location.href='${url('urun-detay')}?id=${u.id}'">
                ...
                <div>
                    ...
                    <div style="font-size:0.8rem; color:var(--ahşap); font-weight:700;">${u.fiyat} ₺</div>
                </div>
            </div>`;
        }).join('');
    }
...
function sayfaKapaklar() {
    ...
    kGrid.innerHTML = '';
    [...kapaklar].sort((a, b) => a.kod.localeCompare(b.kod, 'tr')).forEach(k => {
        ...
        kGrid.innerHTML += `<div class="kapak-kart goster"><img src="${k.resim}" loading="lazy" alt="${kacis(k.kod)}" style="cursor:pointer;" onclick="lightboxAc('${k.resim}')" onerror="this.src='https://via.placeholder.com/300x450?text=KAPAK'"><div class="kapak-kodu">${kacis(k.kod)}</div>${adminHtml}</div>`;
    });
    animasyonlariBaslat();
}
...
function sayfaUrunDetay() {
    ...
    if (yorumListesi) {
        ...
        if (ayarlar.puanGoster && ySayisi > 0) { const ort = urun.yorumlar.reduce((s, y) => s + (y.yildiz || 5), 0) / ySayisi; getEl('detay-fiyat').insertAdjacentHTML('beforebegin', `<div style="font-size:1rem; color:var(--ahşap-acik); margin-bottom:15px;">${yildizCiz(Math.round(ort))} (${ySayisi} Değerlendirme)</div>`); }
        if (ySayisi > 0) {
            urun.yorumlar.forEach((y, index) => {
                ...
                const yildizGoster = ayarlar.puanGoster ? `<span style="color:var(--ahşap-acik);">${yildizCiz(y.yildiz || 5)}</span>` : '';
...
function sayfaSiparislerim() {
    ...
    if (!aktifKullanici) { location.href = url('giris'); return; }
    ...
    [...aktifKullanici.siparisler].reverse().forEach(sip => {
        ...
        const iptalBtn = (sip.durum === 'Hazırlanıyor' || !sip.durum) ? `<button class="btn-satin-al btn-tehlike btn-oto" style="margin-top:10px;" onclick="kullanıcıSiparisIptal('${sip.siparisNo}')">Siparişi İptal Et</button>` : '';
