...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        if (getEl('urun-listesi-izgara')) {
            ...
            if (baslik) {
                const katAd = { 'yatak-odası': 'Yatak Odası', 'mutfak': 'Mutfak', 'calisma-odası': 'Çalışma Odası', 'banyo': 'Banyo' };
...
function sayfaHesabım() {
    ...
    if (favoriListe && aktifKullanici && aktifKullanici.favoriler) {
        ...
        aktifKullanici.favoriler.forEach(id => {
            ...
            favoriListe.innerHTML += `<div class="favori-satir"><img src="${resim}" alt="${kacis(fUrun.ad)}" onerror="this.src='https://via.placeholder.com/60'"><div><h4 style="margin:0;"><a href="${detayBase}?id=${fUrun.id}">${kacis(fUrun.ad)}</a></h4><span style="font-weight:bold; color:var(--ahşap);">${Number(fUrun.fiyat).toLocaleString('tr-TR')} TL</span></div></div>`;
        });
    }
}
    ...
    siraliProjeler.forEach(p => {
        ...
        const adminHtml = yetkiliMi() ? `
            <div style="display: flex; gap: 8px;">
                <button style="background: transparent; border: 1px solid var(--ahşap); color: var(--ahşap); padding: 4px 10px; border-radius: 4px; font-size: 0.75rem; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 5px; transition: all 0.2s;" onmouseover="this.style.background='var(--ahşap)'; this.style.color='#fff';" onmouseout="this.style.background='transparent'; this.style.color='var(--ahşap)';" onclick="projeDuzenleAc(${p.id})">✏️ Düzenle</button>
