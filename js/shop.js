...
function bilgiAlWhatsApp(urunAd, urunId) {
    ...
    const no = ayarlar.wpNo || '905336639714';
...
async function siparisiTamamla() {
    ...
    if (!aktifKullanici) { toastGoster("Sipariş için giris yapın.", "hata"); setTimeout(() => location.href = url('giris'), 1500); return; }
    if (!aktifKullanici.telefon || !aktifKullanici.adres || !aktifKullanici.email) { toastGoster("Lütfen Hesabım sayfasından adres, telefon ve mail bilgilerinizi doldurun.", "hata"); setTimeout(() => location.href = url('hesabim'), 2000); return; }
    ...
    try {
        ...
        setTimeout(() => { location.href = url('siparisler'); }, 2000);
...
function yorumYap(urunId) {
    if (!aktifKullanici) return toastGoster("Yorum yapmak için giris yapmalısınız.", "hata");
...
function favoriEkle(urunId) {
    if (!aktifKullanici) return toastGoster("Kaydetmek için giris yapmalısınız.", "hata");
