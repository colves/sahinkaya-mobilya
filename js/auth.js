...
function sifreDegistir() {
    ...
    if (!user) return toastGoster("Lütfen önce giris yapın", "hata");
    
    const yeni = getEl('yeni-sifre').value, yeniTekrar = getEl('yeni-sifre-tekrar').value;
    if (yeni.length < 6) return toastGoster("Yeni sifre en az 6 hane olmalı!", "hata");
    if (yeni !== yeniTekrar) return toastGoster("Yeni sifreler eşleşmiyor!", "hata");
    ...
    user.updatePassword(yeni).then(() => {
        ...
        if(error.code === 'auth/requires-recent-login') {
            toastGoster("Güvenlik için lütfen çıkış yapıp tekrar giris yapın.", "hata");
        } else {
