/* =========================================================================
   ...
   ========================================================================= */
function iletişimRenderEt() {
    ['imalat', 'mağaza'].forEach(tur => {
        const alan = getEl('iletişim-metin-' + tur); if (!alan) return;
        const info = iletişimBilgileri[tur];
...
/* iletişimDuzenle ve iletişimKaydet admin.js'te tanımlıdır */
    ...
    liste.innerHTML = aktifProjeResimler.map((resim, idx) => {
        ...
        <div class="gorsel-kart" ondragover="projeDragOver(event)" ondrop="projeDrop(event, ${idx})" style="background: var(--kutu-ikinci-bg); padding: 15px; border-radius: var(--radius-sm); position: relative; border: 1px solid var(--kenarlik);">
            ...
            <div style="margin-bottom: 15px; padding: 0 5px;">
                ...
                <input type="range" min="0" max="100" value="${x}" oninput="projeResimOdakKaydir(${idx}, 'x', this.value)" style="width:100%; height:6px; accent-color:var(--ahşap); cursor:pointer; margin-bottom: 20px;">
                ...
                <input type="range" min="0" max="100" value="${y}" oninput="projeResimOdakKaydir(${idx}, 'y', this.value)" style="width:100%; height:6px; accent-color:var(--ahşap); cursor:pointer;">
            </div>
...
function projeResimOdakKaydir(idx, eksen, değer) {
    getEl(`val-${eksen}-${idx}`).innerText = değer + '%';
...
async function projeKaydet() {
    ...
    if (id) { 
        ...
        yeniProje = { ...projeler[i], baslik, aciklama, sira, resimler: resimlerToSave }; 
        projeler[i] = yeniProje;
    } else { 
        yeniProje = { id: Date.now(), baslik, aciklama, sira, resimler: resimlerToSave };
        projeler.push(yeniProje); 
    }
    
    try { 
        toastGoster("Kaydediliyor, lütfen bekleyin...", "basari");
        await db.collection('sistem').doc('projeler').collection('liste').doc(yeniProje.id.toString()).set(yeniProje);
        ...
        console.error("Proje kaydet hatası:", e);
...
function projeSilPanelden(id) { 
    if (!yetkiGerek()) return; 
    ozelOnayGoster("Bu projeyi silmek istediğinize emin misiniz?", async () => { 
        try {
            await db.collection('sistem').doc('projeler').collection('liste').doc(id.toString()).delete();
