...
function sayfayiCiz() {
    ...
    gosterilecekler.forEach(urun => {
        ...
        grid.innerHTML += `<div class="urun-kart goster">
            <div class="urun-kart-resim-kapsam" onclick="location.href='${detayBase}?id=${urun.id}'">
            ...
            <div class="urun-kart-icerik">
                <h3 onclick="location.href='${detayBase}?id=${urun.id}'">${kacis(urun.ad)}</h3>${puanHtml}
                <div class="urun-fiyat" style="margin-bottom: 15px;">${fytMetin}</div>
                <button class="btn-satin-al" style="width:100%; margin-top:auto;" onclick="${btnIslem}" ${btnDisabled}>${btnMetin}</button>
            </div>
        </div>`;
    });
    animasyonlariBaslat();
...
function filtrele() {
    ...
    const stokCbox = getEl('stok-filtre');
    const stokSecim = (stokCbox && stokCbox.checked) ? 'stokta' : 'hepsi';
    const katRadio = document.querySelector('input[name="kat_filtre"]:checked');
    const katSecim = katRadio ? katRadio.value : 'hepsi';
